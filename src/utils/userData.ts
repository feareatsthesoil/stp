...
export async function getUserData(userId: string, force: boolean = false) {
  ...
  const userRow = await prisma.userDataCache.findFirst({
    where: { userId: userId, createdAt: { gte: minsAgo } },
  });
  ...
  if (!userRow || force) {
    ...
    await prisma.userDataCache.create({
      data: { userId, data: { ...user } as any },
    });
    return user;
  } else {
