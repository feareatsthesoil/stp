...
async function meta(req: any, res: NextApiResponse) {
  ...
  const info = await client.contacts.findFirst({where: {userId:userId ?? "Non existent user", profile: true}})
  ...
  return res.status(200).json({info, purchase, message: "Queried succesfully", user: userId, isEdu, isMember:  purchase || isEdu })
}
